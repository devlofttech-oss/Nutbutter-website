...
async function createOrderFromSession(
  ...
  const { data: order, error: orderError } = await serviceClient
    ...
    .insert({
      ...
      estimated_delivery_date: session.estimated_delivery_date,
      metadata: { checkoutSessionId: session.id, merchantOrderId, source: 'phonepe_webhook' },
    })
    .select('*')
    .single()
  ...
  await serviceClient.from('payments').update({ order_id: order.id }).eq('merchant_order_id', merchantOrderId)
  await serviceClient.from('checkout_sessions').update({ status: 'converted', converted_order_id: order.id }).eq('id', session.id)
  await serviceClient.from('cart_items').delete().eq('user_id', session.user_id)
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!signature.ok) {
      await serviceClient.from('phonepe_webhook_events').insert({
    ...
    const { data: payment, error: paymentError } = await serviceClient
      .from('payments')
      .select('id, checkout_session_id, order_id, amount')
    ...
    await serviceClient
      ...
      .update({
        ...
        provider_state: getPhonePeProviderState(statusPayload) || null,
    ...
    if (!success) {
      ...
      await serviceClient
        .from('phonepe_webhook_events')
        .update({ status: 'processed', checkout_session_id: checkoutSessionId, processed_at: new Date().toISOString() })
        .eq('id', eventRow.id)
    ...
    const orderId = payment.order_id || await createOrderFromSession(serviceClient, checkoutSessionId, merchantOrderId)
