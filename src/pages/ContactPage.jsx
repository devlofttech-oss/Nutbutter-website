  ...
  return (
    <div className="bg-background text-on-surface min-h-screen">
      ...
      <main className="min-h-screen">
        ...
        {/* Contact Content */}
        <section className="px-4 sm:px-6 md:px-12 pb-16 md:pb-[120px] max-w-screen-xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-24 items-start">
            {/* Left Column: Details */}
            <div className="space-y-8 md:space-y-12">
              <div className="space-y-6 md:space-y-8">
                {details.map(([icon, title, value]) => (
                  <div key={title} className="flex items-start gap-4 md:gap-6 group">
            ...
            {/* Right Column: Form */}
            <div className="bg-surface-container-low p-5 sm:p-7 lg:p-12 rounded-[22px] md:rounded-[28px] shadow-[0_20px_50px_rgba(140,115,85,0.08)] border border-surface-container-highest/30">
              <form className="space-y-6 md:space-y-8" onSubmit={handleSubmit}>
                ...
                <label className="space-y-2 block">
                  ...
                  <textarea className={`w-full bg-transparent border-0 border-b focus:ring-0 focus:border-tertiary-container transition-colors py-3 px-1 text-base text-on-surface placeholder:text-outline-variant resize-none ${errors.message ? 'border-error' : 'border-outline-variant'}`} placeholder="How can we help you?" rows="4" value={values.message} onChange={(event) => updateValue('message', event.target.value)} />
                  {errors.message && <span className="block text-label-sm text-error">{errors.message}</span>}
                </label>
                <button className="w-full mt-4 bg-primary text-on-primary py-4 md:py-5 px-6 md:px-8 text-xs font-bold uppercase tracking-[0.16em] md:tracking-[0.18em] rounded-full hover:bg-primary-container transition-all duration-300 shadow-md flex justify-center items-center gap-2 disabled:opacity-60" type="submit" disabled={isSubmitting}>
...
function Field({ label, placeholder, type, value, error, onChange }) {
  return (
    <label className="space-y-2 block">
      ...
      <input className={`w-full bg-transparent border-0 border-b focus:ring-0 focus:border-tertiary-container transition-colors py-3 px-1 text-base text-on-surface placeholder:text-outline-variant ${error ? 'border-error' : 'border-outline-variant'}`} placeholder={placeholder} type={type} value={value} onChange={(event) => onChange(event.target.value)} />
