import { useEffect, useState } from 'react'
import { Link, useParams } from 'react-router-dom'
import Header from '../components/Header.tsx'
import Footer from '../components/Footer.tsx'
import OrderTimeline from '../components/OrderTimeline.jsx'
import { fetchOrderById } from '../api/orderApi.js'
import { trackShipment } from '../api/shippingApi.js'

export default function OrderDetailPage() {
  const { orderId } = useParams()
  const [order, setOrder] = useState(null)
  const [isLoading, setIsLoading] = useState(true)
  const [error, setError] = useState('')
  const [isTracking, setIsTracking] = useState(false)

  const loadOrder = async () => {
    const data = await fetchOrderById(orderId)
    setOrder(data)
  }

  useEffect(() => {
    let isMounted = true
    setIsLoading(true)
    fetchOrderById(orderId)
      .then((data) => {
        if (!isMounted) return
        setOrder(data)
        setError(data ? '' : 'Order not found.')
      })
      .catch((fetchError) => {
        if (!isMounted) return
        setError(fetchError.message)
      })
      .finally(() => {
        if (isMounted) setIsLoading(false)
      })

    return () => {
      isMounted = false
    }
  }, [orderId])

  const refreshTracking = async () => {
    setIsTracking(true)
    try {
      await trackShipment(order.id)
      await loadOrder()
      setError('')
    } catch (trackingError) {
      setError(trackingError.message)
    } finally {
      setIsTracking(false)
    }
  }

  return (
    <div className="bg-background text-on-surface min-h-screen flex flex-col">
      <Header />
      <main className="flex-grow max-w-7xl mx-auto w-full px-4 sm:px-6 md:px-12 py-12 md:py-xl">
        <nav className="flex items-center gap-2 mb-sm text-[11px] md:text-xs text-secondary uppercase tracking-[0.12em] md:tracking-widest">
          <Link className="hover:text-primary transition-colors font-serif" to="/">Home</Link>
          <span className="material-symbols-outlined" style={{ fontSize: 14 }}>chevron_right</span>
          <Link className="hover:text-primary transition-colors font-serif" to="/orders">Orders</Link>
          <span className="material-symbols-outlined" style={{ fontSize: 14 }}>chevron_right</span>
          <span className="text-primary font-semibold font-serif">Details</span>
        </nav>

        {isLoading && <State message="Loading order..." />}
        {!isLoading && error && <State message={error} />}
        {!isLoading && order && (
          <section className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
              <div>
                <h1 className="font-serif text-4xl md:text-headline-xl text-primary mb-sm">{order.order_number}</h1>
                <p className="text-on-surface-variant">{new Date(order.created_at).toLocaleString('en-IN')}</p>
              </div>
              <div className="text-left md:text-right">
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-secondary">{order.status.replaceAll('_', ' ')}</p>
                <p className="font-serif text-headline-md text-primary-container mt-1">{formatCurrency(order.total_amount)}</p>
              </div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <article className="lg:col-span-7 rounded-xl border border-outline-variant bg-white/40 p-5 md:p-6">
                <h2 className="font-serif text-2xl text-primary mb-5">Items</h2>
                <div className="space-y-sm">
                  {(order.order_items ?? []).map((item) => (
                    <div key={item.id} className="flex justify-between gap-4 text-base md:text-body-md text-on-surface-variant">
                      <span>{item.product_name} x {item.quantity}</span>
                      <span>{formatCurrency(item.line_total)}</span>
                    </div>
                  ))}
                </div>
              </article>

              <article className="lg:col-span-5 rounded-xl border border-outline-variant bg-surface-container-low p-5 md:p-6">
                <h2 className="font-serif text-2xl text-primary mb-5">Timeline</h2>
                <OrderTimeline events={order.order_timeline_events ?? []} />
              </article>
            </div>

            {order.shipments?.[0] && (
              <article className="rounded-xl border border-outline-variant bg-surface-container-low p-5 md:p-6 text-on-surface-variant">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.18em] text-secondary">Shipping</p>
                    <p className="mt-2">{order.shipments[0].courier_name || order.shiprocket_courier_name || 'Shiprocket'} / {order.shipments[0].status.replaceAll('_', ' ')}</p>
                    {order.shipments[0].awb_code && <p className="mt-1">AWB: {order.shipments[0].awb_code}</p>}
                    {(order.shipments[0].estimated_delivery_date || order.estimated_delivery_date) && (
                      <p className="mt-1">Estimated delivery: {new Date(order.shipments[0].estimated_delivery_date || order.estimated_delivery_date).toLocaleDateString('en-IN')}</p>
                    )}
                  </div>
                  <button
                    className="w-full md:w-auto rounded-full border border-primary px-5 py-3 text-xs font-bold uppercase tracking-[0.14em] text-primary transition-all hover:bg-primary hover:text-on-primary disabled:opacity-60"
                    type="button"
                    disabled={!order.shipments[0].awb_code || isTracking}
                    onClick={refreshTracking}
                  >
                    {isTracking ? 'Refreshing...' : 'Refresh Tracking'}
                  </button>
                </div>
              </article>
            )}
          </section>
        )}
      </main>
      <Footer />
    </div>
  )
}

function State({ message }) {
  return (
    <section className="bg-surface-container rounded-xl border border-outline-variant p-8 md:p-xl text-center text-on-surface-variant">
      {message}
    </section>
  )
}

function formatCurrency(value) {
  return new Intl.NumberFormat('en-IN', {
    style: 'currency',
    currency: 'INR',
    maximumFractionDigits: 0,
  }).format(Number(value) || 0)
}
